...
import codeExample from '../examples/GeometryCrop.vue?raw'
...
const labels = computed(() => isEn.value ? {
  ...
  description: 'Flip the image, adjust zoom limits, and read the selected corners in source-image coordinates.',
  ...
  title: '几何编辑',
  description: '翻转图片、调整缩放范围，观察裁剪区域对应的源图坐标。',
  interaction: '交互控制',
  minScale: '最小缩放比例',
  maxScale: '最大缩放比例',
  ...
  <section class="geometry-page">
    ...
    <section class="geometry-demo">
      ...
      <section class="control-column">
        <section class="panel">
          <h2 id="interaction">{{ labels.interaction }}</h2>
          <section class="field">
            <span>{{ labels.minScale }}</span>
            <el-input-number v-model="minScale" :aria-label="labels.minScale" :min="0.01" :max="10" :step="0.1" />
          </section>
          <section class="field">
            <span>{{ labels.maxScale }}</span>
            <el-input-number v-model="maxScale" :aria-label="labels.maxScale" :min="0.01" :max="10" :step="0.1" />
          </section>
          <section class="field">
            <el-switch
              v-model="zoomAnchor"
              ...
              :aria-label="labels.zoomAtPointer" :active-text="labels.zoomAtPointer"
            />
          </section>
        ...
        <section class="result-grid">
          <section class="panel result-panel">
            <h2 id="state">{{ labels.state }}</h2>
            <pre>{{ stateText }}</pre>
          </section>
          <section class="panel result-panel">
            <h2 id="coordinates">{{ labels.coordinates }}</h2>
            <pre>{{ coordinatesText }}</pre>
          </section>
        </section>
      </section>
    </section>
    <p class="integration-note">{{ isEn ? 'Copy this complete component into your Vue project. Use your own photo.jpg or select a local file.' : '把下面的完整组件复制到 Vue 项目，放入自己的 photo.jpg，或选择本地图片。' }}</p>
    <ScenarioCodeExample :code="codeExample" :title="isEn ? 'Complete integration example' : '完整接入示例'" :copy-label="isEn ? 'Copy code' : '复制代码'" :copied-label="isEn ? 'Copied' : '已复制'" />
  </section>
</template>
